        ...
        success: function(data, status){
                ...
                if(data.length == 0){
                    ...
                    if(jQuery.mobile.activePage[0].id == "main"){
                        showMessage('No se encontraron promos. Intenta nuestra bÃºsqueda manual.', 'Info', 'Ok');
                        event.preventDefault();
                        gotoSearch();
                        return;
                    }
...
function getGeoLocation(){
	$.mobile.showPageLoadingMsg('a', "Buscando tu localización...", false);
...
function onError_highAccuracy(error) {
    var msg = "";
    //if(error.code == 2)
        //msg = 'No se pudo obtener datos del GPS. Se localizarÃ¡ por 3G, por lo que la localizaciÃ³n puede presentar un desvÃ­o de 150 mts aproximadamente.';
